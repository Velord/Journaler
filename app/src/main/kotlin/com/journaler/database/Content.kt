...
object Content: CRUD<DbEntry> {
    ...
    override fun delete(what: List<DbEntry>, clazz: KClass<DbEntry>): Int {
        val db = DbHelper(Content.name, Content.version).writableDatabase
        db.beginTransaction()
        ...
        if (clazz.simpleName == Note::class.simpleName){
            ...
            if (success) {
                db.setTransactionSuccessful()
                Log.i(Content.tag, "Delete [$table] [SUCCESS] [$count] [$statement]")
            }
            else
                Log.w(Content.tag, "Delete [$table] [ FAILED] [$statement]")
